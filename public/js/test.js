  $(document).ready(function() {
    ...
    //########## function to extract paramaters from url ##########################
    ...
    function nextQuestion(i) {
      if (timeLimit) {
        clearInterval(timeinterval);
        var deadline = new Date(Date.parse(new Date()) + timeLimitVal * 1000);
    ...
    //prepare URL for quiz API
    var id = getUrlParameter("id");
    var url = '/api/quiz/' + id;

    //retrive quiz data from api
    $.getJSON(url, function(data) {
      ...
      if (questionsNum == 'all' || questionsNum >= qNum) {
        maxQuestion = qNum;
      }
      else {
